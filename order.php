...
$query = "SELECT o.Orderhdr_id, o.Created_dt, c.CustomerName, 
                 ...
                 SUM(CAST(p.Price AS DECIMAL(10,2)) * CAST(od.Quantity AS UNSIGNED) as TotalAmount
          FROM Order_hdr o
...
const sidebar=document.getElementById('sidebar'),mobileToggle=document.getElementById('mobileMenuToggle'),body=document.body;
mobileToggle&&mobileToggle.addEventListener('click',function(e){e.stopPropagation(),sidebar.classList.toggle('active'),body.classList.toggle('sidebar-open')});
document.addEventListener('click',function(e){window.innerWidth<=992&&!sidebar.contains(e.target)&&(!mobileToggle||e.target!==mobileToggle)&&(sidebar.classList.remove('active'),body.classList.remove('sidebar-open'))});
document.querySelectorAll('.sidebar-item').forEach(e=>{e.addEventListener('click',function(){window.innerWidth<=992&&(sidebar.classList.remove('active'),body.classList.remove('sidebar-open'))})});
window.addEventListener('resize',function(){window.innerWidth>992&&(sidebar.classList.remove('active'),body.classList.remove('sidebar-open'))});
const customerSearch=document.getElementById('customerSearch');customerSearch&&customerSearch.addEventListener('input',function(){const e=this.value.toLowerCase();document.querySelectorAll('.customer-select-table tbody tr').forEach(t=>{const n=t.querySelector('td:nth-child(2)').textContent.toLowerCase(),a=t.querySelector('td:nth-child(3)').textContent.toLowerCase(),i=t.querySelector('td:nth-child(1)').textContent.toLowerCase();n.includes(e)||a.includes(e)||i.includes(e)?t.style.display="":t.style.display="none"})});
document.querySelectorAll('.select-customer').forEach(e=>{e.addEventListener('click',function(){const e=this.getAttribute('data-customer-id'),t=this.getAttribute('data-customer-name'),n=bootstrap.Modal.getInstance(document.getElementById('addOrderModal'));n.hide(),window.location.href=`orderCreate.php?customer_id=${e}`})})});
</script>
