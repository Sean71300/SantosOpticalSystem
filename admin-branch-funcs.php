...
function addBranch() {
    ...
    mysqli_close($link);
    // Post-Redirect-Get to avoid refresh/resubmit prompt
    if ($result) {
        header('Location: admin-branch.php?added=1');
        exit();
    } else {
        header('Location: admin-branch.php?added=0');
        exit();
    }
}
...
function confirmDeleteBranch() {
    ...
    // PRG
    if ($result) {
        header('Location: admin-branch.php?deleted=1');
        exit();
    } else {
        header('Location: admin-branch.php?deleted=0');
        exit();
    }
}
