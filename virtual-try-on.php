...
<head>
  ...
  <!-- Using a different face-api.js CDN -->
...
<body>
  ...
  <script>
    ...
    // Replace with your actual glasses image path
    ...
    async function loadModels() {
      ...
      try {
        ...
        if (typeof faceapi === 'undefined') {
          throw new Error('face-api.js not loaded properly');
        }
        ...
        try {
          ...
          modelsLoaded = true;
          updateStatus(modelsStatus, modelsText, 'ready', 'Loaded');
          console.log("Face detection models loaded successfully (alternative method)");
          return true;
        } catch (altError) {
          ...
          showError("Failed to load face detection models. Please check your internet connection and refresh the page.");
          return false;
        }
      }
    }
      ...
      detectionInterval = setInterval(async () => {
        try {
          ...
          if (detections) {
            ...
            // Adjusted for better fit - make glasses taller
    ...
    // Check if faceapi is available on page load
    window.addEventListener('load', () => {
      console.log("Page loaded, checking face-api.js...");
      
      if (typeof faceapi !== 'undefined') {
        console.log("face-api.js is available");
        updateStatus(modelsStatus, modelsText, 'not-ready', 'Ready to Load');
      } else {
        console.error("face-api.js is not available");
        updateStatus(modelsStatus, modelsText, 'not-ready', 'Library Missing');
